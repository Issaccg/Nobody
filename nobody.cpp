...
int main(int argc, char **argv) {
    if (argc != 2) {
        ...
        const unsigned int n = atoi(argv[l]);
        offset_momentum ( state) ;

        out_stream. open ("nodycpp·csv");
        out_stream<<"body;xpos;ypos;zpos"<<std::endl;
        int statesize = sizeof(state)/sizeof (state[0]);
        for ( int i= 0; i<n; ++i) {
            for ( int j=0; j<stateSize; j++) {
                out_stream<<state[j].name <<";";
                out_stream<<state[j].position.x<<";";
                out_stream<<state[j].position.y<<";";
                out_stream<<state[j].position.z<<";"<< std::endl;
            }
            advance(state,0.01);
        } 
        out_stream.close();
